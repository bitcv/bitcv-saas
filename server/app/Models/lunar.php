...
class lunar extends Model
{
    ...
    function convertSolarToLunarSimple($date)
    {
        $time = strtotime($date);
        $year = date("Y", $time);
        $month = date("m", $time);
        $day = date("d", $time);
        return $this->convertSolarToLunar($year, $month, $day);
    }
    /**
     * 将阳历转换为阴历
     * @param year 公历-年
    ...
    function getLunarMonthName($lunaryear, $monthorder)
    {
        $lunar_month_table = array(
        ...
        $firstword =trim(mb_substr($lunaryear, 0, 1));
        $months =explode(",",$lunar_month_table[$firstword]);
        return $months[$monthorder-1];
    }
    function getLunarDayName($date)
    {
        $skyland_table = "甲子,乙丑,丙寅,丁卯,戊辰,己巳,庚午,辛未,壬申,癸酉,
	甲戌,乙亥,丙子,丁丑,戊寅,己卯,庚辰,辛巳,壬午,癸未,
	甲申,乙酉,丙戌,丁亥,戊子,己丑,庚寅,辛卯,壬辰,癸巳,
	甲午,乙未,丙申,丁酉,戊戌,己亥,庚子,辛丑,壬寅,癸卯,
	甲辰,乙巳,丙午,丁未,戊申,己酉,庚戌,辛亥,壬子,癸丑,
	甲寅,乙卯,丙辰,丁巳,戊午,己未,庚申,辛酉,壬戌,癸亥";
        $startdate = strtotime("1949-10-1");
        $enddate = strtotime($date);
        $days = ($enddate - $startdate)/86400;
        $days = $days % 60;

        $skyland = explode(",", $skyland_table)[$days];
        return $skyland;
    }
